        ...
        {weeks.map((week, index) => (
          <div key={week.id} className="space-y-2">
            <div className="flex items-center justify-between">
              <span className={`text-sm font-medium ${week.isActive ? 'text-white' : 'text-white/60'
                }`}>
                {week.id}
              </span>
              {week.isActive && (
            ...
            <div className="relative">
              <div className="w-full h-2 bg-white/20 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all duration-500 ${week.isActive ? 'bg-gradient-to-r from-orange-500 to-orange-400' : 'bg-white/30'
                    }`}
